...
本教學課程會使用[!DNL Adobe App Builder]、[!DNL Edge Delivery Services]和AI輔助開發工具，引導您建立[!DNL Adobe Commerce as a Cloud Service]的傳遞日期估計延伸。 此擴充功能會從外部API擷取運送時間和運送日期的預估值，並在整個店面顯示。
...
1. Deploy the workflow.
   ...
   Your endpoint is the webhook trigger URL.
...
The App Builder Runtime action option deploys the mock as a Runtime action within the same [!DNL App Builder] project. This approach has no external dependency and no invocation quotas.
...
Authentication is validated against a `MOCK_API_KEY` environment variable in `.env`.
...
1. From the MCP settings in your coding agents, verify that the `commerce-extensibility` toolset is enabled without errors.

   For example, in Cursor, go to **[!UICONTROL Cursor]** > **[!UICONTROL Settings]** > **[!UICONTROL Cursor Settings]** > **[!UICONTROL Tools & MCP]**.

   If you see errors, toggle the toolset off and on.
...
The agent returns with a series of clarifying questions covering topics such as target environment (PaaS vs SaaS), scope (standalone BFF action, webhook enrichment, or both), origin address source, caching strategy, and testing preference. 下列範例顯示典型的問題和答案。 您的代理可能會問不同的問題，但主題通常相同。
...
>[!NOTE]
>
>For SaaS, the webhook method name drops `magento.` from the path. The webhook name for PaaS is `plugin.magento.out_of_process_shipping_methods...` while webhook name for SaaS is `plugin.out_of_process_shipping_methods...`.
...
1. Configure extensions by selecting your [!DNL App Builder] workspace and extension.

1. 按一下&#x200B;**[!UICONTROL Refresh registrations]**。

1. Navigate to **[!UICONTROL Apps]** > **[!UICONTROL Delivery Estimates]** in the [!DNL Admin] sidebar.

1. Complete the configuration by enabling the feature and specifying the required settings, including the API URL and API key, origin address, default carriers, cache TTL, and carrier code mappings.
...
The agent produces `docs/STOREFRONT_API_SPEC.md` with the full BFF endpoint contract (request and response format, example payloads, error handling) and a ready-to-use prompt for the storefront workspace.

Copy this file into your storefront project before starting the storefront dev steps.
...
| `aio app deploy`失敗，因為「productDependencies中需要maxVersion」 | CLI驗證需要`app.config.yaml`產品相依性中的`minVersion`和`maxVersion`。 | 將`maxVersion`值新增至`app.config.yaml`中的每個`productDependencies`專案。 |
...
| 首次載入時未顯示結帳傳送日期 | `checkout/updated`事件接聽程式是在`checkout/initialized`觸發後註冊，因此遺漏了初始資料。 | 新增具有`{ eager: true }`的`checkout/initialized`接聽程式，以擷取註冊前發出的事件。 保留`checkout/updated`接聽程式以進行後續變更。 |
...
| 統一運費在結帳時不會顯示交貨日期 | 依設計 — `CARRIER_MAP`僅將DPS對應至標準，並將Fedex對應至表示。 統一費率在外部API中沒有對應的電信業者。 | 不是錯誤。 若要新增其他電信業者的預估，請延伸`scripts/delivery-estimates.js`中的`CARRIER_MAP`，並在後端延伸中設定該電信業者。 |
