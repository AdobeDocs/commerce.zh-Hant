...
>[!TIP]
>
>若要同時新增多個使用者，您可以執行[大量CSV上傳](https://helpx.adobe.com/business/enterprise/users/users-and-groups/bulk-upload-users.html){target="_blank"}。
>
> 您也可以建立[使用者群組](https://helpx.adobe.com/business/enterprise/users/users-and-groups/user-groups.html){target="_blank"}，將多位使用者新增至角色。 然後，您可以將適當的產品新增到使用者群組。
...
* **使用者** — 使用者擁有Commerce管理員的管理員存取權，但無法在Admin Console中管理產品層級的存取權。 使用者也可以使用積分在[!DNL Commerce Cloud Manager]中[建立執行個體](./getting-started.md#create-an-instance)。
  ...
  >[!TIP]
  >
  >如果您想要依IP位址限制對Commerce管理員的存取權，請參閱[依IP位址限制產品存取權](https://helpx.adobe.com/business/enterprise/identity-sso/set-up-identity/restrict-product-access-by-ip-address.html){target="_blank"}。

* [**開發人員**](https://helpx.adobe.com/business/enterprise/users/users-and-groups/manage-developers.html#Adddevelopers){target="_blank"} — 開發人員擁有使用者許可權，並且已作為開發人員使用者新增至Commerce執行個體。 他們可以使用[[!DNL Admin UI SDK]](https://developer.adobe.com/commerce/extensibility/admin-ui-sdk/){target="_blank"}、[設定事件](https://developer.adobe.com/commerce/extensibility/events/){target="_blank"}和[建立Webhook](https://developer.adobe.com/commerce/extensibility/webhooks/){target="_blank"}。

* 管理員 — 有不同型別的管理員角色：
  * [系統管理員](https://helpx.adobe.com/business/enterprise/users/manage-admins/admin-roles-hierarchy.html){target="_blank"} — 系統管理員可以透過Admin Console存取組織中的所有產品和產品設定檔。
  * [產品管理員](#add-a-product-admin) — 產品管理員可以在[!DNL Adobe Admin Console]中[管理產品的使用者、角色和許可權](#add-users)，並在Commerce管理員中[管理使用者](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/user-accounts/permissions-users-all#create-a-user){target="_blank"}。
...
根據貴組織的布建時間，有兩種不同的方式可將產品管理員使用者新增到Adobe Commerce as a Cloud Service。 在早期存取組織中，獲指派產品管理員角色的每個使用者都有權管理組織中的所有執行個體。 在2025年10月13日之後布建的「一般可用性」(GA)組織中，您可以將使用者指派為特定執行個體的產品管理員。 當產品管理員使用者登入時，他們只能看到他們有權管理的執行個體。
...
>[!NOTE]
>
>沒有適當資產許可權的使用者無法存取[!DNL AEM Assets]的進階功能，例如[AI影像產生](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/overview){target="_blank"}、[產生的變數](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/generate-variations/generate-variations-integrated-editor){target="_blank"}等等。

>[!TIP]
>
>若要同時新增多個使用者，您可以執行[大量CSV上傳](https://helpx.adobe.com/business/enterprise/users/users-and-groups/bulk-upload-users.html){target="_blank"}。
>
>您也可以建立[使用者群組](https://helpx.adobe.com/business/enterprise/users/users-and-groups/user-groups.html){target="_blank"}，將多位使用者新增至角色。 然後您可以將&#x200B;[!DNL **Adobe Experience Manager as a Cloud Service**]&#x200B;產品新增到使用者群組。

1. 導覽至<https://adminconsole.adobe.com>並使用您的Adobe ID登入。

1. 選取您的組織。

1. 在&#x200B;[!UICONTROL **產品**]&#x200B;標籤的&#x200B;[!UICONTROL **產品和服務**]&#x200B;下，選取&#x200B;[!UICONTROL **Adobe Experience Manager as a Cloud Service**]&#x200B;產品。
...
若要確認使用者是否具有存取權，請按一下使用者的名稱以開啟其設定檔頁面。 在&#x200B;[!UICONTROL **產品**]&#x200B;區段中，[!UICONTROL **已完成**]&#x200B;顯示在&#x200B;[!UICONTROL **Adobe Experience Manager as a Cloud Service**]&#x200B;產品下。 新增使用者後，需要幾秒鐘才能看到其設定檔上的狀態更新。 重新整理頁面以檢視更新的狀態。
...
1. [建立新環境](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-manager/content/requirements/users-and-roles){target="_blank"}。
...
1. 您現在應該會看到新的產品設定檔。 選取包含`- author -`的專案。 例如，`<environment-name> - author - <program-id> - <environment-id>`。
