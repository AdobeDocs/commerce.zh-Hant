...
每個詳細目錄來源現在都包含[!DNL Commerce Admin]中[來源編輯頁面](https://experienceleague.adobe.com/en/docs/commerce-admin/inventory/sources/sources-add)的&#x200B;[!UICONTROL **顯示於店面**]&#x200B;切換（[!UICONTROL **商店**] > [!UICONTROL **詳細目錄**] > [!UICONTROL **來源**]）。 [`sourceAvailability`](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/source-availability) GraphQL查詢只會傳回您標示為可見之來源的庫存資訊。 預設會隱藏來源。<!-- ACCS-1645 -->

### 多來源出貨指南

當訂單包含指派給不同存貨來源的專案時，[!DNL Commerce Admin] [!UICONTROL **Source選擇**]&#x200B;頁面現在會自動引導您為每個專案選取適當的來源。<!-- ACCS-1832 -->

### 查詢店面的來源可用性詳細資料

[`sourceAvailability`](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/source-availability) GraphQL查詢現在會傳回店面的其他資訊：

* 每個來源都包含其`name`和`is_pickup_location_active`旗標，因此店面可以顯示來源名稱並識別使用中的店內取貨位置。<!-- ACCS-1614 -->

* `SourceAvailability`型別現在包含`is_saleable`和`backorder_message`欄位，因此店面可以將[!UICONTROL Add to Cart]引向真實銷售性，而非實體庫存。<!-- ACCS-1858 -->

### 擷取客戶許可權

新的[!DNL Commerce] REST端點(`GET /V1/customers/:customerId/companyRoles`)傳回所有指派給客戶的公司角色和許可權。 GraphQL `CompanyBasicInfo`型別現在也包含`role_id`、`role_name`和許可權資料，因此整合功能可以在單一要求中，擷取所有關聯帳戶中使用者的公司角色指派。<!-- ACCS-1617 -->

### 訂閱商業發票儲存webhook

`observer.sales_order_invoice_save_after` webhook可讓您在儲存商業發票之後執行邏輯。<!-- CEXT-6706 -->

### 檢視指派的來源資訊

當訂單包含具有指定存貨來源的專案時，[!DNL Commerce Admin]現在會在訂單檢視頁面和出貨來源選取畫面中標籤這些專案，讓商家可以履行來自正確來源的訂單。<!-- ACCS-941 -->

### 增強功能和錯誤修正

此版本中包括下列選取的增強功能、最佳化和錯誤修正：

* 已最佳化購物車和結帳層級價格載入，以防止大量共用目錄發生問題。<!-- ACCS-1150 -->

* 修正未遵循檔案屬性變更的問題。<!-- CCSAAS-5395 -->

* 修正產品覆寫資料不一致的問題。<!-- ACCS-1844 -->

* 修正並行的REST API請求可能會間歇性導致401錯誤的問題。<!-- CCSAAS-5417 -->

* 修正當未設定明確時間時，購物車價格規則的開始或結束日期可能意外運作的問題。<!-- ACCS-1856 -->

* 修正購物車價格規則接受不存在的開始或結束日期的問題。<!-- ACCS-1902 -->

* 修正無法儲存[!UICONTROL Catalog]設定的問題。<!-- CCSAAS-5436 -->

* 修正同時要求匯入API (`POST /V1/import/json`)時可能發生的問題。<!-- ACCS-1053 -->

* 修正可銷售數量可能多報指定存貨來源之產品可用性的問題。<!-- ACCS-1103 -->

* 修正訂單註解API (`POST /V1/orders/{id}/comments`)不區分大小寫的問題。<!-- ACCS-1752 -->
...
### Adobe Commerce as a Cloud Service更新至2.4.9

[!DNL Adobe Commerce as a Cloud Service]現在包含來自[!DNL Adobe Commerce]版本2.4.9的所有變更。

如需詳細資訊，請參閱[Adobe Commerce 2.4.9發行說明](https://experienceleague.adobe.com/en/docs/commerce-operations/release/notes/adobe-commerce/2-4-9)。
...
這些端點可讓管理員以程式設計方式跨環境同步處理設定，而非手動重新設定[!DNL Commerce Admin]。 在您的沙箱環境中執行`GET /V1/system/config`，然後使用先前擷取的沙箱組態執行`PUT /V1/system/config`，可讓您將組態變更從沙箱同步到生產環境。<!-- ACCS-607, CCSAAS-5346 -->
...
[啟用&#x200B;**每個Source的可用性**](https://experienceleague.adobe.com/en/docs/commerce-admin/inventory/configuration/global-options)。<!-- ACCS-933 -->
...
新的`PUT`和`DELETE` [自訂電子郵件](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/custom-email/)範本端點可讓整合更新和刪除自訂電子郵件範本。<!-- CCSAAS-5091 -->

### 透過REST API檢視產品選項群組和識別碼

`GET /V1/products/:sku/options` REST呼叫現在會傳回每個選項的`group`和`option_uids`欄位，符合透過GraphQL已可取得的識別碼。<!-- ACCS-1370 -->

### 新增共用的目錄事件

下列共用目錄事件現在可以使用[!DNL Adobe I/O Events]訂閱：<!-- ACCS-1532 -->

* 類別指派(`observer.shared_catalog_assign_categories`)
* 類別取消指派(`observer.shared_catalog_unassign_categories`)
* 公司指派(`plugin.magento.shared_catalog.api.company_management.assign_companies`)
* 公司取消指派(`plugin.magento.shared_catalog.api.company_management.unassign_companies`)
* 公司解除指派 — 全部(`plugin.magento.shared_catalog.api.company_management.unassign_all_companies`)
* 共用目錄儲存(`plugin.magento.shared_catalog.api.shared_catalog_repository.save`)
* 共用目錄刪除(`plugin.magento.shared_catalog.api.shared_catalog_repository.delete`)

### 跨採購單、報價和退貨使用公司地址

公司通訊錄現在與其他B2B工作流程整合。 使用共用通訊錄的公司會看到以下各項一致、公司範圍的地址：
...
### 在[!DNL AEM Assets]中隱藏存放區檢視中的影像

[!DNL AEM Assets]整合現在支援`hiddenStoreViews`引數，因此匯入的影像在特定存放區檢視中可將範圍設定為隱藏。 這可讓您向不同的地區或人口統計店面顯示不同的產品影像。<!-- ACAP-1308 -->

### 在網站範圍加入PayPal帳戶

商戶現在可以直接從[!DNL Commerce Admin]在網站範圍自助服務不同PayPal帳戶的上線。 Payment Services首頁現在包含&#x200B;**為網站連線其他PayPal帳戶**&#x200B;按鈕，該按鈕會重新導向至付款方法的管理員設定頁面。 如需詳細資訊，請參閱[為網站](https://experienceleague.adobe.com/en/docs/commerce/payment-services/configure/connect-website-account)連線其他PayPal帳戶。<!-- PAY-6961 -->
...
您現在可以設定要[購物車價格規則](https://experienceleague.adobe.com/en/docs/commerce-admin/marketing/promotions/cart-rules/price-rules-cart-create#rule-information)在[!DNL Commerce Admin]中開始或結束的當日時間。 購物車價格規則格線會顯示排程時間，而REST API會遵循在`from_date`和`to_date`上提交的時間，而不是將規則設定為午夜。<!-- ACCS-970 -->
...
* 修正使用註冊客戶的電子郵件下訪客訂單時可能發生的問題。<!-- CCSAAS-5313 -->

* 修正重複資料匯出執行可能導致資源問題的問題。<!-- CCSAAS-5275 -->

* 修正[!DNL AEM Assets]整合中GraphQL媒體集標籤的遞補問題。<!-- ACAP-1308 -->

* 修正[!DNL PayPal]個SDK引數可能影響簽出轉譯的問題。<!-- PAY-6961 -->

* 修正結帳時可能顯示不支援的[!DNL Payment Services]付款方法的問題。<!-- PAY-6976 -->

* 修正具有陣列值欄位的事件裝載（例如共用目錄類別和公司指派事件）可能包含空白物件，而非預期資料的問題。<!-- CEXT-6554 -->

* 修正設定數個可搜尋的客戶或客戶地址屬性可能會造成顯示錯誤的問題。 客戶格線現在會在您達到限制時通知您。<!-- CCSAAS-5303 -->
...
所有`custom-email`端點都需要存取`Marketing > Communications > Email template` [角色資源](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/user-accounts/permissions-user-roles#step-2assign-resources)。<!-- CCSAAS-5089, CCSAAS-5090 -->
...
新的[電子郵件隱藏](https://experienceleague.adobe.com/en/docs/commerce-admin/config/services/email-suppression)設定（[!UICONTROL **商店**] > [!UICONTROL **設定**] > [!UICONTROL **Adobe服務**] > [!UICONTROL **電子郵件隱藏**]）可讓系統管理員選擇性停止[!DNL Commerce]傳送異動電子郵件。 您可以依功能區域（客戶帳戶、Order Management、退貨、結帳、行銷或B2B）或範本識別碼的精確清單來隱藏電子郵件。<!-- ACCS-1025 -->
...
商戶現在可以直接從手動購物車價格規則上的[!DNL Commerce Admin] [建立和編輯自訂優惠券代碼](https://experienceleague.adobe.com/en/docs/commerce-admin/marketing/promotions/cart-rules/price-rules-cart-coupon#method-3-custom-coupon-codes)。 編輯購物車價格規則時，[!UICONTROL **管理優惠券代碼**]&#x200B;區段中提供新的&#x200B;[!UICONTROL **新增自訂優惠券**]&#x200B;按鈕。<!-- CCSAAS-4508 -->
...
[!UICONTROL **編輯**]&#x200B;按鈕現在可在&#x200B;[!UICONTROL **訂單檢視**]&#x200B;畫面中用於部份開立商業發票的訂單，讓商家有更多彈性可修改仍在處理中的訂單。 以前，即使未開立商業發票的專案仍然存在，也無法編輯包含任何商業發票的訂單。 只要訂單中的任何專案仍可開立商業發票，就可以編輯訂單。 擁有自訂整合且依賴先前編輯限制的商家應檢閱其工作流程。<!-- ACCS-849 -->

### 增強功能和錯誤修正

此版本中包括下列選取的增強功能、最佳化和錯誤修正：

* 修正`GET /V1/products` REST API清單端點未傳回`stock_item`擴充功能屬性的問題。 回應現在與記錄的API合約相符。<!-- ACCS-819 -->

* 修正電子郵件中密碼重設連結傳回404錯誤的問題。<!-- ACCS-797 -->

* 已針對使用日期範圍篩選器的公司改善訂單歷史記錄查詢效能。<!-- ACCS-859 -->

* 修正B2B公司使用者在使用者加入公司前可看見來自的對等訂單的問題。<!-- ACCS-859 -->

* 解決在載入已啟用`trigger_recollect`的引號時，可能會影響REST API效能的簽出逾時問題。<!-- CCSAAS-4904 -->

* 修正在[!DNL Commerce Admin]中提交訂單後可能發生的頁面載入問題。<!-- CCSAAS-4413 -->

* 修正具有相同時間戳記的訂單可能會在銷售訂單網格中顯示過期訂單狀態資訊的問題。<!-- CCSAAS-4890 -->
...
* 新增`GET /V1/order-statuses` REST API端點以擷取所有已設定的順序狀態及其狀態指派。<!-- CEXT-6100 -->

* 解決造成Order、Cart、Invoice、Credit Memo和Company等實體的`custom_attributes`無法顯示在REST結構描述中的問題。<!-- CCSAAS-4818 -->

* 解決非同步大量API取用者中的重複訊息處理錯誤(`MessageLockException`)。<!-- CCSAAS-4805 -->

* 為篩選選項啟用屬性時，產品屬性數量現在會在[!DNL Commerce Admin]產品格線中顯示為起始/終止範圍篩選器。<!-- ACCS-761 -->

* 修正購物車放棄提醒電子郵件在使用[!DNL AEM Assets]時未顯示產品影像的問題。<!-- ACCS-798 -->

* 修正將檔案、範例或連結新增至可下載產品時，可能顯示誤判的「最大上傳大小」錯誤的問題。<!-- ACCS-813 -->

* 修正儲存指派給數個共用目錄的產品可能會造成錯誤的問題。<!-- ACCS-788 -->

* 修正訂單歷史記錄查詢速度可能緩慢，並會導致許多訂單的公司出現資料庫記憶體不足錯誤的問題。<!-- ACCS-808 -->

* 修正匯入檔案驗證可能失敗的問題。<!-- CCSAAS-4364 -->

* 已從「**[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**」中的「**[!UICONTROL Catalog]**」區段中移除「**[!UICONTROL Recently Viewed/Compared Products]**」設定，因為它在[!DNL Adobe Commerce as a Cloud Service]管理員中不受支援。<!-- ACCS-793 -->
...
以下專案已於2026年4月16日發佈到生產環境。

>[!BEGINSHADEBOX]

### 使用Webhook實作自訂報價總計

`plugin.magento.out_of_process_totals_collector.api.get_total_modifications.execute` [webhook](https://developer.adobe.com/commerce/extensibility/webhooks/)現在可在[!DNL Adobe Commerce as a Cloud Service]中使用。 透過程式外的擴充功能，使用它來實作自訂報價總計修改。<!-- CEXT-5896 -->
...
* 修正「共用目錄指派公司」頁面上的500錯誤，此錯誤可能在將大型共用目錄與Admin UI SDK搭配使用時發生。<!-- CCSAAS-4783 -->

* 修正當客戶在指派給公司前下訂單時，公司客戶無法看到自己的訂單的問題。<!-- ACCS-746 -->
...
以下專案已於2026年4月1日發佈到生產環境。

>[!BEGINSHADEBOX]

### 將檔案新增至產品

[!DNL Adobe Commerce as a Cloud Service]現在支援[使用檔案型別產品屬性將檔案新增至產品](./product-files.md)。 您可以在產品編輯頁面上手動上傳檔案、以程式設計方式透過REST API，或透過以CSV提供外部URL來大量上傳。<!-- ACCS-535, ACCS-565 -->
...
* 您現在可以使用適用的自訂屬性來篩選「訂單」和「公司REST API」結果，支援公司範圍訂單搜尋等案例。<!-- ACCS-633 -->

* 解決可能出現在瀏覽器開發人員控制檯中的錯誤。<!-- CCSAAS-4650 -->

* 修正取消含有訂單註解的客體訂單時可能發生的錯誤。<!-- ACCS-674 -->

* 修正將具有多個關聯專案的已分組產品新增至請購單清單時可能發生的錯誤。<!-- ACCS-627 -->
...
[電子郵件提醒模組](https://experienceleague.adobe.com/en/docs/commerce-admin/marketing/communications/email-reminders/email-reminder-rules) (`Magento_Reminder`)目前在[!DNL Adobe Commerce as a Cloud Service]中處於使用中。 它可讓商家建立自動提醒規則，以根據購物車和願望清單閒置狀態觸發傳送給客戶的電子郵件。<!-- CCSAAS-4597 -->
...
新的選用商店層級設定可讓客戶[追蹤他們進行的訪客訂單](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/point-of-purchase/checkout/checkout-guest#allow-guest-order-access-for-registered-emails)。 如果訂單使用的電子郵件地址符合註冊的客戶帳戶，則適用此規定。<!-- ACCS-289 -->
...
* 解決由於與資產角色相關的自訂屬性中`no_selection`值損毀，[!DNL Edge Delivery Services]中無法正確顯示產品影像的問題。<!-- ACAP-1206 -->
...
您現在可以[將使用者存取權](./user-management.md#add-users)指派給Admin Console中的個別[!DNL Adobe Commerce as a Cloud Service]執行個體。<!-- CCSAAS-4337 -->
<!-- See PR #332 -->

### 可觀察性

透過[!DNL App Builder]，您可以透過[OpenTelemetry可觀察性](https://developer.adobe.com/commerce/extensibility/observability/)更深入瞭解您的[!DNL Adobe Commerce as a Cloud Service]執行個體，現在可自動使用。 OpenTelemetry提供量度、日誌和追蹤，協助您監控效能、更快地疑難排解問題，以及最佳化店面。 此功能可讓您主動深入分析系統健康狀況，並提升客戶的可靠性。
