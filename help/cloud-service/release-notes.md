...
以下專案將於2026年4月1日發佈到生產環境。

>[!BEGINSHADEBOX]

### 透過GraphQL檢查價格和庫存警示訂閱狀態

新的GraphQL查詢`isSubscribedProductAlertStock`和`isSubscribedProductAlertPrice`，讓店面判斷購物者是否已訂閱產品的庫存或價格警示。<!-- ACCS-334 -->
...
`recaptchaFormConfigs`查詢可以在單一請求中傳回多個表單型別的組態，而不需要每個表單都有一個查詢。<!-- ACCS-628 -->
