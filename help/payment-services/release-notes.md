...
以下版本說明說明[!DNL Payment Services]的最新版本。

Adobe支援最新發行的支付服務版本。 舊版的發行說明僅供參考。
...
![新問題](../assets/new.svg)<!-- Issue PAY-6510 -->對於[!DNL Adobe Commerce as a Cloud Service]，[!DNL Commerce Admin]中的[!DNL Payment Services]儀表板UI可讓商家直接從儀表板開啟[交易報表](reporting.md#transactions-report-view)。 此SaaS儀表板只會公開&#x200B;**選取的**&#x200B;報告（不是雲端和內部部署上Adobe Commerce上可用的每個報告或首頁進入點）；請參閱[[!DNL Payment Services] 首頁](payments-home.md)。 如需[!DNL Adobe Commerce as a Cloud Service]發行說明和時間，請參閱[檢視付款服務交易報表](../cloud-service/release-notes.md#view-the-payment-services-transactions-report)。

_2026年1月21日_

![新問題](../assets/new.svg)<!-- Issue PAY-6374 -->現在，[!DNL Payment Services]儀表板上的&#x200B;**設定**&#x200B;按鈕重新導向到付款方法的管理員設定頁面，提供更精簡的付款設定管理工作流程。

_2026年1月19日_

![新問題](../assets/new.svg)<!-- Issue PAY-6325 -->現在，商戶可以在[交易報表](reporting.md#transactions-report-view)中以欄的形式檢視PayPal賣家識別碼，以協助識別特定客戶已進行的交易。
...
![新](../assets/new.svg)<!-- Issue PAY-4050 -->已新增出現在付款服務首頁的[資料視覺化檢視](payouts.md#payouts-data-visualization-view)，以取得訂單付款狀態報告。

![已修正問題](../assets/fix.svg)<!-- Issue PAY-4486-->之前，英國商戶的結帳功能中並未顯示PayPal PayLater按鈕。 該問題已解決。

![已修正問題](../assets/fix.svg)<!-- Issue PAY-4485-->當[!DNL Payment Services]停用時，[!DNL Payment Services]首頁現在會出現報告資料視覺效果檢視。
...
![新的](../assets/new.svg)<!-- Issue PAY-1720 -->商店訂單糾紛現在可在[訂單付款狀態報表](order-payment-status.md#view-disputes)中取得。 您可以從[!DNL Payment Services]直接導覽至PayPal解決中心來解決爭議。
...
![新增](../assets/new.svg)<!-- Issue PAY-2761 -->您現在可以使用[資料行]設定控制項來顯示或隱藏資料行，以自訂[訂單付款狀態報表](order-payment-status.md#show-and-hide-columns)與[付款報表](payouts.md#show-and-hide-columns)中顯示的資料。
...
![新增](../assets/new.svg)<!-- PAY-6564 -->現在，[!DNL Google Pay]快速結帳可以使用使用者端送貨回撥在Google付款單中顯示送貨方法，讓購物者先選取送貨方式，再確認付款。 商家可以在[Google付款設定](configure-admin.md#google-pay)中使用&#x200B;**[!UICONTROL Skip Review]**&#x200B;來顯示或隱藏個別的Google付款稽核步驟。 如果略過複查步驟，當公事包地址或送貨方法變更時，送貨選項會保留在工作表中。

![新](../assets/new.svg)<!-- PAY-6565 -->現在，無論是否出現Google付款稽核步驟，購物者都可以在[!DNL Google Pay]快速付款表中套用促銷代碼。 每個工資單只能套用一個代碼，即使商店允許每張訂單使用多張抵用券。 如需詳細資訊，請參閱[付款選項](payments-options.md#google-pay-button)。

![新](../assets/new.svg)<!-- PAY-6567 -->現在，除了Safari之外，Chrome、Firefox和Microsoft Edge也提供[!DNL Apple Pay]快速簽出。 在桌上型電腦上，購物者可使用執行iOS 18或更新版本的iPhone掃描二維碼，以完成Apple Pay表中的付款。 必須在[Apple Pay設定](configure-admin.md#apple-pay)中啟用快速版位（例如，在產品詳細資料頁面上）。

![新增](../assets/new.svg)<!-- PAY-6566 -->現在，購物者可以在[!DNL Apple Pay]速遞工資表中套用或移除單一折扣代碼，並在表中更新總計。 照常在「管理員」中建立並啟用購物車價格規則。

![新](../assets/new.svg)<!-- PAY-6151 -->現在，Luma產品詳細資料和結帳頁面上的[!DNL Apple Pay]會透過Payments SDK呈現，因此體驗符合用於快速結帳的最新PayPal整合。

![新](../assets/new.svg)<!-- PAY-6611 -->現在，[!DNL Adobe Commerce]訂單詳細資料頁面會顯示透過[!DNL Payment Services]處理之交易的額外付款詳細資料，包括[!DNL PayPal]交易ID、PayPal偵錯ID、付款人電子郵件、商家保護資格及AVS/CVV驗證代碼。 如需詳細資訊，請參閱[3DS](security.md#3ds)。

![已修正問題](../assets/fix.svg)<!-- PAY-6513 -->修正付款嘗試失敗且稍後成功結帳後，[!DNL Apple Pay]錯誤訊息仍可能顯示在訂單成功頁面的問題。

![已修正問題](../assets/fix.svg)<!-- PAY-6585 -->修正國家/地區不是必要欄位的UK及歐洲地址中，[!DNL Apple Pay]快速工資單顯示&#x200B;**[!UICONTROL Please select a state/region]**&#x200B;錯誤的問題。
...
![New](../assets/new.svg)<!-- PAY-6400 -->已新增&#x200B;**[!UICONTROL Line Items Enabled]**&#x200B;設定，以啟用或停用傳送行專案及金額劃分至PayPal。 此設定可協助解決協力廠商擴充功能新增[!DNL Payment Services]不支援的自訂費用時的簽出問題。 如需詳細資訊，請參閱[條列專案](line-items.md)。

![新增](../assets/new.svg)<!-- PAY-6457 -->在結帳開始時新增[快速付款按鈕](payments-options.md#express-checkout-buttons)，以鼓勵更快速的結帳。

![新](../assets/new.svg)<!-- PAY-6458 -->已針對特定地區與當地付款選項，新增對[當地付款方法(LPM)](payments-options.md#local-payment-methods)的支援。 商家可以直接在[Commerce](configure-admin.md#local-payment-methods)中啟用或停用可用的LPM。

![新](../assets/new.svg)<!-- PAY-6377 -->已新增[購買者的國家](sandbox.md#buyers-country)沙箱設定，以模擬購買者的位置並控制要轉譯的付款方式。

![已修正問題](../assets/fix.svg)<!-- PAY-6458 -->藉由傳遞買家電子郵件地址資料，改善與PayPal詐騙篩選器的相容性，進而增強詐騙防護功能。

![已修正問題](../assets/fix.svg)<!-- PAY-6413 -->修正因遺失運送地址而無法完成虛擬產品PayPal簽出的問題。

![已修正問題](../assets/fix.svg)<!-- PAY-6450 -->修正產品詳細資料頁面、迷你圖、購物車和結帳頂端未正確處理PayPal按鈕結帳錯誤的問題。

![已修正問題](../assets/fix.svg)<!-- PAY-6453 -->已在GraphQL簽出時停用伺服器端送貨回呼流程，以防止在簽出時發生衝突。

![已修正問題](../assets/fix.svg)<!-- PAY-6456 -->已更新[Apple Pay](configure-admin.md#apple-pay)設定說明，以澄清Apple會拒絕來自未驗證網域的付款。 商家必須驗證任何使用Apple Pay按鈕的網域。 國家限制適用。

## v2.13.3

_2026年1月14日_

![已修正問題](../assets/fix.svg)<!-- PAY-6399 -->已修正管理員結帳期間使用錯誤儲存卡的問題。 現在，下訂單時可正確使用選取的卡片。
...
![新增](../assets/new.svg)<!-- PAY-6022 --> [Fastlane](https://experienceleague.adobe.com/zh-hant/docs/commerce/payment-services/payments-checkout/payments-options)在訪客結帳期間提供更快的購買速度。
...
![新增](../assets/new.svg)<!-- PAY-6227 -->使用PayPal結帳時，[!DNL Payment Services]會略過訂單確認快顯視窗，以加快購買程式。 這是由[伺服器端送貨回呼](payments-options.md#server-side-shipping-callbacks-for-paypal-payment-buttons)所啟用，它直接在PayPal檢閱頁面中計算送貨與總計。

![新](../assets/new.svg)<!-- PAY-6234 -->已新增[稍後付款](https://experienceleague.adobe.com/zh-hant/docs/commerce/payment-services/payments-checkout/payments-options)付款選項的新功能。 現在，BNPL訊息設定程式可在客戶結帳頁面上顯示「稍後付款」BNPL訊息，提供更大的彈性。
...
![修正](../assets/fix.svg)<!-- PAY-5792 --> [!DNL Payment Services]現在會在結帳頁面[&#128279;](vaulting.md)上顯示儲存卡的說明和帳單地址，讓客戶更清楚瞭解他們儲存的付款方式。
...
![新增](../assets/new.svg)<!-- PAY-4844 --> [!DNL Payment Services]現在支援訂單層級[&#128279;](line-items.md)的條列專案資料。 此功能可讓商家檢視訂單中專案的詳細資訊，例如產品詳細資料、數量及價格（包括銷售稅、折扣及其他相關資訊）。

![新增](../assets/new.svg)<!-- PAY-5380 --> [!DNL Payment Services]改善商家在Admin[&#128279;](configure-admin.md#general-configuration)體驗中的設定，以更簡單且更直覺的方式上線流程。 此功能可讓商家重設其[!DNL Payment Services] ID。
...
![新的](../assets/new.svg)<!-- PAY-4877 -->現在，[!DNL Payment Services]支援[L2/L3定價功能](levels-card-payment-transactions.md#level-2-and-level-3)。 此功能僅適用於已啟用IC++定價的[!DNL Payment Services]客戶。 如果您想要使用L2/L3處理[!DNL Payment Services]的資料，請連絡您的[!DNL Payment Services]帳戶管理員。
...
![新的](../assets/new.svg)<!-- PAY-4868 -->商戶可透過管理員成功[設定Google Pay的整個購買體驗](payments-options.md)，類似於[!DNL Payment Services]中的其他付款按鈕。

![新增](../assets/new.svg)<!-- PAY-4381 --> [Payment Services透過GraphQL支援Google Pay](https://developer.adobe.com/commerce/webapi/graphql/payment-services-extension/)，讓商戶可以使用Google Pay付款方式取得Headless Commerce體驗。

![新增](../assets/new.svg)<!-- PAY-4878 -->現在，[!DNL Payment Services]基本簽出功能已針對Adobe Commerce和Magento Open Source商家整合。[!DNL Payment Services] 現在可以支援業務遍及全球200個地區的商家。[!DNL Payment Services] 基本結帳功能在自助入門中提供借記/貸記、PayPal、Venmo （可用時）和PayLater （可用時）選項。

![修正](../assets/fix.svg)<!-- PAY-5291 -->某些交易的收款確認可能延遲。 在這種情況下，商家現在可以獲得訂單的更新付款狀態。 [付款服務會偵測訂單中付款交易](order-payment-status.md#payment-status-updates)的暫緩狀態，方法是偵測暫緩交易並主動監視這些交易，並在擷取暫緩狀態時進行更新。
...
- 店面的客戶訂單頁面
- 訂單確認電子郵件已傳送給購物者
- 從Commerce管理員的[訂單詳細資料檢視](https://experienceleague.adobe.com/zh-hant/docs/commerce-admin/stores-sales/order-management/orders/order-processing#view-an-order)。
...
![新增](../assets/new.svg)<!-- PAY-4381 --> [付款服務現在支援GraphQL整合](https://developer.adobe.com/commerce/webapi/graphql/payment-services-extension/)。 透過GraphQL對PayPal付款按鈕、託管欄位和Apple Pay的支援，[!DNL Payment Services]現在可支援Headless Commerce設定。
...
![New](../assets/new.svg)<!-- PAY-4638 -->已新增與Signifyd[&#128279;](fraud-protection.md)的整合，此整合提供自動化的詐騙防護服務。
...
![新](../assets/new.svg)<!-- PAY-4035 -->商戶現在可以使用[!DNL Payment Services] Admin首頁提供的新[交易報告](reporting.md#transactions-report-view)，有效監控其商店的健康狀況，並識別任何交易問題。 此報表也會顯示有關交易授權率及負數交易趨勢的資料。
...
![新](../assets/new.svg)<!-- Issue xxx -->已在下列國家和相關貨幣中新增[可用性](compatibility.md#standard-vs-advanced-payment-services-experience)：澳洲、法國、英國。
...
![修正](../assets/fix.svg)<!-- Issue PAY-4226 -->修正新[!DNL Payment Services]商家無法在Admin中使用簽出功能的問題。[!DNL Payment Services] 之前使用Commerce客戶ID，新客戶沒有此ID。
...
![修正](../assets/fix.svg)<!-- Issue PAY-4202 -->修正客戶無法使用`Authorize and Capture`付款動作[&#128279;](production.md#set-payment-services-as-payment-method)，使用卡儲存庫完成購買或刪除商店的儲存庫付款方式的問題。 以前，當客戶嘗試使用或修改其存放的信用卡時，會出現「找不到提供者儲存庫ID」錯誤。
...
![新](../assets/new.svg)<!-- Issue PAY-3950 -->商戶現在可以為其商店啟用[立即購買Commerce功能](https://experienceleague.adobe.com/zh-hant/docs/commerce-admin/stores-sales/point-of-purchase/checkout-instant-purchase)，讓購物者可以（使用[保管式信用卡資訊](vaulting.md)）加快結帳速度。
...
![已知問題](../assets/bug.svg)<!-- Issue PAY-0 --> [!DNL Payment Services]與`commerce-data-export` v101.20和更新版本不相容，這會使其與[[!DNL Channel manager] 擴充功能](https://experienceleague.adobe.com/zh-hant/docs/discontinued/using/commerce-channels) （已停止）不相容。
...
![新](../assets/new.svg)<!-- Issue PAY-2682 -->加拿大商戶現在可以使用[!DNL Adobe Commerce]和[!DNL Magento Open Source]的[!DNL Payment Services]延伸模組。 商戶可以使用[法文](compatibility.md##standard-vs-advanced-payment-services-experience)或[英文](compatibility.md#standard-vs-advanced-payment-services-experience)檢視付款組態。

![新增](../assets/new.svg)<!-- Issue PAY-2681 --> [!DNL Payment Services]在信用卡與PayPal交易上支援[加幣(CAD)](compatibility.md#standard-vs-advanced-payment-services-experience)。
...
![已修正問題](../assets/fix.svg)<!-- Issue PAY-2277 -->您現在可以選擇性地啟用或停用個別PayPal付款按鈕[&#128279;](configure-admin.md#paypal-payment-buttons)。

![已修正問題](../assets/fix.svg)<!-- Issue PAY-2561 -->先前移除的產品未出現在&#x200B;_檢閱訂單_&#x200B;頁面的購物車中。

![已知問題](../assets/bug.svg)<!-- Issue PAY-2842 -->在沙箱環境中處理付款時，透過PayPal測試信用卡交易[可能會失敗](https://experienceleague.adobe.com/zh-hant/docs/experience-cloud-kcs/kbarticles/ka-26825)。
...
![新](../assets/new.svg)<!-- Issue PAY-124 -->可在雲端基礎結構[&#128279;](install.md#adobe-commerce-on-cloud-infrastructure)或[內部部署](install.md#on-premises-and-other-configurations)執行個體上為[!DNL Adobe Commerce] 安裝[!DNL Adobe Commerce]和[!DNL Magento Open Source]的[!DNL Payment Services]延伸模組。 這些方法需要使用指令行介面。
...
![已知問題](../assets/bug.svg)<!-- Issue PAY-2473 -->在安裝擴充功能期間使用[不正確的撰寫器金鑰](https://experienceleague.adobe.com/zh-hant/docs/experience-cloud-kcs/kbarticles/ka-26826)，會導致使用者無法[使用正確的`MAGEID`驗證](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/installation-guide/prerequisites/authentication-keys)。

![已知問題](../assets/bug.svg)<!-- Issue PAY-2474 --> [!DNL Payment Services]報告[可能無法立即同步](https://experienceleague.adobe.com/zh-hant/docs/experience-cloud-kcs/kbarticles/ka-26837)。

![已知問題](../assets/bug.svg)<!-- Issue PAY-2475 -->如果您在上線期間建立您的[!DNL Payment Services]的[PayPal沙箱帳戶](https://experienceleague.adobe.com/zh-hant/docs/experience-cloud-kcs/kbarticles/ka-26836)，則無法驗證該帳戶。
