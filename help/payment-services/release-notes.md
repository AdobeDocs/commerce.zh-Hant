...
![修正](../assets/fix.svg)<!-- PAY-5792 --> [!DNL Payment Services]現在會在結帳頁面](vaulting.md)上顯示[儲存卡的說明和帳單地址，讓客戶更清楚瞭解他們儲存的付款方式。
...
![新增](../assets/new.svg)<!-- PAY-5380 --> [!DNL Payment Services]改善商家在Admin](configure-admin.md#general-configuration)體驗中的[設定，以更簡單且更直覺的方式上線流程。 此功能可讓商家重設其[!DNL Payment Services] ID。
...
- 店面的客戶訂單頁面
- 訂單確認電子郵件已傳送給購物者
- 從Commerce管理員的[訂單詳細資料檢視](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/order-management/orders/order-processing#view-an-order)。
...
![New](../assets/new.svg)<!-- PAY-4638 -->已新增與Signifyd](fraud-protection.md)的[整合，此整合提供自動化的詐騙防護服務。
...
![修正](../assets/fix.svg)<!-- Issue PAY-4202 -->修正客戶無法使用`Authorize and Capture`付款動作](production.md#set-payment-services-as-payment-method)，使用卡儲存庫完成購買或刪除商店[的儲存庫付款方式的問題。 以前，當客戶嘗試使用或修改其存放的信用卡時，會出現「找不到提供者儲存庫ID」錯誤。
...
![新](../assets/new.svg)<!-- Issue PAY-3950 -->商戶現在可以為其商店啟用[立即購買Commerce功能](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/point-of-purchase/checkout-instant-purchase)，讓購物者可以（使用[保管式信用卡資訊](vaulting.md)）加快結帳速度。
...
![已知問題](../assets/bug.svg)<!-- Issue PAY-0 --> [!DNL Payment Services]與`commerce-data-export` v101.20和更新版本不相容，這會使其與[[!DNL Channel manager] 擴充功能](https://experienceleague.adobe.com/en/docs/discontinued/using/commerce-channels) （已停止）不相容。
...
![已知問題](../assets/bug.svg)<!-- Issue PAY-2842 -->在沙箱環境中處理付款時，透過PayPal測試信用卡交易[可能會失敗](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26825)。
...
![已知問題](../assets/bug.svg)<!-- Issue PAY-2473 -->在安裝擴充功能期間使用[不正確的撰寫器金鑰](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26826)，會導致使用者無法[使用正確的`MAGEID`驗證](https://experienceleague.adobe.com/en/docs/commerce-operations/installation-guide/prerequisites/authentication-keys)。

![已知問題](../assets/bug.svg)<!-- Issue PAY-2474 --> [!DNL Payment Services]報告[可能無法立即同步](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26837)。

![已知問題](../assets/bug.svg)<!-- Issue PAY-2475 -->如果您在上線期間建立您的[!DNL Payment Services]的[PayPal沙箱帳戶](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26836)，則無法驗證該帳戶。
