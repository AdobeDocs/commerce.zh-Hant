...
IMS驗證預設為啟用。 將使用者新增至[Adobe Admin Console](https://adminconsole.adobe.com/)中的&#x200B;**AEM Assets DM OpenAPI使用者 — 傳遞**&#x200B;產品設定檔，或新增至&#x200B;**作者**&#x200B;產品設定檔（例如，`<environment-name> - author - <program-id> - <environment-id>`），當使用者的Admin Console中沒有AEM傳遞產品設定檔時，作為遞補。
...
1. [為Commerce啟用Adobe IMS](https://experienceleague.adobe.com/docs/commerce-admin/start/admin/ims/adobe-ims-config.html){target=_blank}，讓Commerce管理員使用IMS驗證，並可讀取使用者的Admin Console產品設定檔。
