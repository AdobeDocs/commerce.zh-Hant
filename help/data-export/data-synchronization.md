...
SaaS資料匯出會持續將產品資料從Adobe Commerce執行個體移至Commerce Services平台，以保持資料在最新狀態。 例如，產品建議需要目前的目錄資訊，才能正確地傳回具有正確名稱、價格和可用性的建議。 如需監視同步處理程式的詳細資訊，請參閱[檢視和管理同步處理程式](#view-and-manage-the-synchronization-process)。
...
對[!DNL Adobe Commerce Optimizer Connector]摘要執行相同的部分同步cron工作。 如需聯結器特定的提交與錯誤處理，請參閱[聯結器同步管道](../aco-connector/connector-sync-pipeline.md)。
...
- [!BADGE 僅限PaaS]{type=Informative url="https://experienceleague.adobe.com/zh-hant/docs/commerce/user-guides/product-solutions" tooltip="僅適用於雲端專案（Adobe管理的PaaS基礎結構）和內部部署專案的Adobe Commerce 。"} **[資料管理儀表板](https://experienceleague.adobe.com/zh-hant/docs/commerce-admin/systems/data-transfer/data-sync/data-dashboard)** — 管理員使用者可以檢視及追蹤同步處理至Commerce Services且可供店面服務使用的資料。 此儀表板會顯示同步至Commerce Services的產品。

  {{aco-data-sync-verification}}

- [!BADGE 僅限SaaS]{type=Positive url="https://experienceleague.adobe.com/zh-hant/docs/commerce/user-guides/product-solutions" tooltip="適用於與Adobe Commerce Optimizer整合的Adobe Commerce專案（Adobe管理的SaaS基礎結構）。"} **[資料同步摘要同步狀態頁面](https://experienceleague.adobe.com/zh-hant/docs/commerce/optimizer/setup/data-sync)** — 對於使用[!DNL Adobe Commerce Optimizer]的Commerce專案，請從Adobe Commerce Optimizer的資料摘要同步狀態頁面檢查店面的目錄資料可用性。 此儀表板會顯示資料匯出摘要的同步狀態。

>[!NOTE]
>
>只有在您已安裝「即時搜尋」、「產品建議」或「目錄服務」時，才能使用「資料管理」控制面板。 如果您已安裝這些服務或[Adobe Commerce Optimizer聯結器](../aco-connector/overview.md)，即可使用資料摘要同步狀態儀表板。 如需Optimizer聯結器管線行為，包括範圍控制及提交錯誤，請參閱[聯結器同步管道](../aco-connector/connector-sync-pipeline.md)。
