...
![修正](../assets/fix.svg) **貨幣變更後的產品摘要更新** — 當您變更商店貨幣設定時，受影響的產品現在會再次排入匯出佇列，因此「即時搜尋」、「目錄服務」、「產品建議」和其他連線的Commerce服務會顯示更新的價格。<!--MDEE-1376-->

![修正](../assets/fix.svg) **子類別反映停用的父項狀態** — 當您停用最上層類別時，子類別現在會匯出為非使用中，因此連線的Commerce服務不會將這些類別中的產品視為可探索。<!--MDEE-1369-->
...
![修正](../assets/fix.svg) **遺失預設範圍資料時的可靠狀態和可見性** — 產品匯出會處理預設管理範圍上的間隙，以便連線的服務能夠正確啟用、停用和可見性行為。 您設定的存放區特定值仍優先。

<!--MDEE-1341-->

![修正](../assets/fix.svg) **在多個商店群組間遵循目錄許可權** — 在複雜的商店設定中，允許的產品可繼續在即時搜尋和相關體驗中尋找適當的客戶群組，而不是在許可權規則衝突時隱藏。

<!--MDEE-1334-->

![修正](../assets/fix.svg) **在完整摘要重設之前更清楚的警告** — 將`saas:resync`與`--cleanup-feed`選項搭配使用現在可說明本機匯出狀態已重設，且SaaS可能會與您的目錄不同步。 此選項僅適用於完整環境重建，而非例行疑難排解。
...
![修正](../assets/fix.svg) **類別移動後固定類別路徑匯出** — 確保類別移至其他父級時，類別摘要能正確更新`url_path`，避免連線的Commerce服務遺失或過時的類別路徑。<!--MDEE-1331-->
![修正](../assets/fix.svg) **已改善相關產品的已排程類別更新** — 類別URL的已排程更新現在只會影響預期的類別，可保留資料完整性並防止相關產品發生意外變更。現在，排程的類別URL變更會正確反映在匯出的資料中，讓店面導覽和連結的服務與目前的目錄保持一致。
<!--MDEE-1321-->
...
![修正](../assets/fix.svg)解決遺失`cde_product_overrides_feed_cl`資料表時[mview索引子](https://developer.adobe.com/commerce/php/development/components/indexing/#mview)工作可能失敗的問題。 此修正可確保穩定地重新索引，並防止多租使用者環境中與此表格相關的工作失敗。<!--MDEE-1175-->
