...
* [驗證金鑰](https://experienceleague.adobe.com/en/docs/commerce-operations/installation-guide/prerequisites/authentication-keys)，以使用Composer下載Commerce Connector中繼套件。
...
1. **[使用Composer安裝Commerce Optimizer聯結器套件](#install-the-commerce-optimizer-connector-package)**，以將您的Commerce執行個體連線至[!DNL Adobe Commerce Optimizer]。

1. **[從管理員檢閱及自訂資料匯出設定](#customize-the-commerce-data-export-configuration)**。

1. **[取得在Commerce和Commerce Optimizer之間建立連線所需的API認證](#get-required-connection-details)**。
...
從Adobe Developer Console建立啟用[!DNL Adobe Commerce Optimizer]內嵌服務的開發人員專案，並產生OAuth伺服器對伺服器認證。 如需詳細指示，請參閱&#x200B;*銷售開發人員指南*&#x200B;中的[取得IMS認證](https://developer.adobe.com/commerce/services/optimizer/data-ingestion/authentication/#obtain-ims-credentials)。
...
| 範圍 | 資料已匯出 | 附註 |
| ------- | --------------- | ------- |
| 網站 | 價格與價格手冊 | 每組價格會使用命名慣例`website::customergroupcode`匯出為[價格簿](../optimizer/setup/pricebooks.md)。 包括網站的所有客戶群組。 |
...
使用您[在先前步驟](#get-required-connection-details)中收集的OAuth伺服器對伺服器認證和執行個體詳細資訊，您現在可以設定Commerce與[!DNL Adobe Commerce Optimizer]執行個體之間的整合。
...
您可以從Admin中可用的[資料摘要同步狀態](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/data-transfer/data-sync/data-feed-sync-status)頁面監視及驗證同步是否正常運作。
...
1. **設定[!DNL Adobe Commerce Optimizer]目錄檢視與原則**
   ...
   在[!DNL Adobe Commerce Optimizer] UI中建立目錄檢視和原則。 請注意，價格簿是從Adobe Commerce客戶群組自動建立的。 如需指示，請參閱&#x200B;*Commerce Optimizer使用手冊*&#x200B;中的[目錄檢視](../optimizer/setup/catalog-view.md)和[原則](../optimizer/setup/catalog-view.md)檔案。
