...
- 未重試&#x200B;**400個錯誤**。 檢查裝載中是否有格式錯誤或遺漏的必填欄位。 如需預期的格式，請參閱聯結器摘要](reference/field-mapping.md)的[欄位對應。
...
如需底層[!DNL SaaS Data Export]診斷（包括記錄位置與摘要重新同步命令），請參閱[[!DNL SaaS Data Export] 疑難排解指南](https://experienceleague.adobe.com/en/docs/commerce/saas-data-export/logs-troubleshooting/troubleshooting-logging){target="_blank"}。
