...
▢生產執行個體作用中。請參閱[開始使用](../get-started.md)，瞭解如何布建它。
▢執行個體在正確的區域中。
▢環境型別為「生產」。
...
[匯出組態](../../aco-connector/get-started.md)中的▢同步處理範圍符合您的需求。
▢ [資料摘要同步處理狀態](../../aco-connector/data-sync-manage.md)會確認從雲端執行個體匯出資料。
...
▢ [價格手冊](../setup/pricebooks.md)是從雲端上的客戶群組自動建立的。
▢ [目錄檢視](../setup/catalog-view.md)存在，而且您知道它們的ID。
▢ [原則](../setup/policies.md)存在，而您知道它們的ID。
...
▢ [價格多面向和搜尋語言](../settings.md)符合您使用這些功能的需求。
▢ [產品建議](../merchandising/recommendations/create.md)存在，且行為符合預覽中的預期。
▢ [搜尋效能資料](../manage-results/search-performance.md)出現在&#x200B;*管理員*中。
▢ [Recommendations效能資料](../manage-results/recommendation-performance.md)會顯示在&#x200B;*管理員*&#x200B;中。
...
▢ **架構：**&#x200B;主商務系統處理付款、運送、稅捐及存貨（非[!DNL Adobe Commerce Optimizer]）。
▢ **架構：** API Mesh和App Builder在主機商務系統與[!DNL Adobe Commerce Optimizer]之間保持同步。
▢ **電子郵件：**異動電子郵件傳送可正常運作（訂單確認、送貨等）。
▢ **電子郵件：**&#x200B;電子郵件範本符合您的品牌並使用正確的連結。
...
▢地區設定會顯示適用的正確翻譯頁面。
▢其他HTML標籤視需要[已設定](https://www.aem.live/docs/redirects)。
...
▢ **SSL：**已安裝信任的SSL/TLS憑證。
▢ **SSL：** HTTPS已在整個網站強制執行。
▢ **存取：**&#x200B;預設&#x200B;*管理員*&#x200B;密碼已變更，而且已設定強式密碼原則。請參閱[!DNL Adobe Commerce Optimizer] [使用者和身分管理](../user-management.md)。
▢ **存取：** *管理員* URL不是預設值。
已針對所有*管理員*&#x200B;使用者啟用▢ **存取：**雙因素驗證。
▢ **存取：**沒有非作用中或未使用的管理員使用者與專案相關聯。
▢ **防火牆：** Web應用程式防火牆(WAF)已設定並驗證。
▢ **PCI：**生產環境的安全性滲透測試（PCI範圍）已完成。
▢ **掃描：** Adobe安全性掃描工具已註冊，且初始掃描已完成。
▢ **存取：** CORS僅允許已核准的原始項。
