...
**健全狀態檢查**&#x200B;索引標籤提供每個行為事件健全狀態的深入分析，以確保精確的資料收集和功能。 &#x200B;
...
_Cold Start_&#x200B;問題是指模型訓練及生效所需的時間。 針對建議，這表示等候Adobe AI收集足夠的資料以訓練其機器學習模型，然後再將建議單位部署在您的網站上。 模型擁有的資料越多，建議就越準確和有用。 由於資料收集會在即時網站上進行，因此最好及早開始此程式。
...
下列事件並非建議所特有，但Adobe AI必須具備，才能正確解譯購物者資料：
...
| Bought this, bought that | `page-view`<br>`product-view` | 產品詳細資料頁面 |
| Trending | `page-view`<br>`product-view` | 產品詳細資料頁面 |
| Conversion: View to purchase | `page-view`<br>`product-view` | 產品詳細資料頁面 |
| Conversion: View to purchase | `page-view`<br>`place-order` | 購物車/結帳 |
