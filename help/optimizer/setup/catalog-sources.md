...
目錄來源代表產品、屬性和類別的權威範圍。 通常會對應至語言、對象或原始系統邊界，並決定搜尋、篩選和排序行為。

## 目錄來源與相關概念

瞭解目錄來源與其他[!DNL Adobe Commerce Optimizer]概念的關聯性，可協助您正確建立資料的模型：
...
* 每個目錄來源都是透過資料擷取API擷取產品所建立。 如需詳細資訊，請參閱[開發人員檔案 — 資料擷取](https://developer.adobe.com/commerce/services/optimizer/data-ingestion/)。
* 產品唯一性由SKU +目錄來源決定。
