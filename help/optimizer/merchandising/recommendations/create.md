...
1. 在&#x200B;**目錄檢視**&#x200B;欄位中，選取您要讓建議可用的目錄檢視。 深入瞭解[使用建議](../../manage-results/recommendation-performance.md#select-catalog-view)的目錄檢視。
   ...
   >[!IMPORTANT]
   >
   >此功能目前在[beta](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/release/beta#merchandising-rules-globally-and-per-catalog-view-public-beta)中。 Beta參與者將需要重新建立任何現有的建議單位，以善用新的目錄檢視範圍。
...
- 足夠的結果集大小：大多數案例中傳回的結果是否足夠避免使用[備份建議](../../setup/events/overview.md#backup-recommendations)？
...
- 少於50%表示為該建議型別建議的建議可能無關緊要。 在此情況下，會使用[備份建議](../../setup/events/overview.md#backup-recommendations)。

深入瞭解[為何整備程度指標可能較低](#what-to-do-if-the-readiness-indicator-is-low)。
...
低整備百分比表示您的目錄中沒有許多產品符合納入此建議型別之建議中的資格。 這表示，如果仍要部署此建議型別，很可能傳回[備份建議](../../setup/events/overview.md#backup-recommendations)。
