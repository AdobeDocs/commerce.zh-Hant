...
本教學課程的最後一個部分涉及更新[您已建立](#prerequisites)的店面，以將資料傳送至新的Celport目錄。 在本節中，您會以Celport的目錄檢視ID取代店面設定檔案中的目錄檢視ID。
...
1. 將`ac-price-book-id`值取代為`"east_coast_inc"`。
   ...
   進行這些變更後，您的`config.json`檔案應該看起來類似下列內容，將`commerce-optimizer-tenant-id`和`celport-catalog-view-id`預留位置取代為您的值：
...
1. 嘗試更新您的店面組態檔(`config.json`)。
   ...
   這些實驗展示了[!DNL Adobe Commerce Optimizer]的彈性 — 您可以快速切換不同的目錄檢視與價格簿，為不同的對象建立自訂購物體驗，而不需要複製您的目錄資料。
