...
使用[資料管理儀表板](https://experienceleague.adobe.com/zh-hant/docs/commerce-admin/systems/data-transfer/data-sync/data-dashboard.md)監視同步處理進度。 此儀表板提供您店面產品資料可用性的寶貴見解，確保可及時向客戶顯示。
...
如果您未使用標準元件(例如搜尋轉接器、Luma Widget或AEM CIF Widget)，預設將無法自動收集使用者互動資料。 Adobe AI會將此收集到的資料用於智慧型銷售和效能追蹤。 若要解決此問題，您需要開發自訂解決方案，以透過Headless方式實施此資料收集。
