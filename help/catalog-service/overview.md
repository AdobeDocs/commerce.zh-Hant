...
* **目錄事件**：透過[!DNL Adobe I/O Events]發佈目錄變更通知，讓整合功能無需輪詢GraphQL即可對產品、類別和價格更新做出反應
...
當[!DNL Catalog Service]中的目錄資料變更時，目錄事件會透過[!DNL Adobe I/O Events]通知訂閱的消費者。 使用這些事件讓快取失效、更新搜尋索引或同步外部系統，而不輪詢GraphQL API。 如需事件型別、傳遞保證和設定，請參閱[目錄事件和Adobe I/O整合指南](catalog-events-guide.md)。
...
1. **更新店面程式碼**：將目錄服務GraphQL查詢整合至您的店面。
1. **路由查詢**：所有目錄服務查詢都會透過GraphQL閘道（上線期間提供的URL）
1. **監視和疑難排解資料同步處理**：驗證已改善的效能並監視結果
1. **（選擇性） [設定目錄事件](catalog-events-guide.md)** — 設定[!DNL Adobe I/O Events]訂閱以接收快取失效、搜尋索引或外部系統同步處理的目錄變更通知。
